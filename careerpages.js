...
// Create a new company (admin version)
router.post('/admin/company', verifyAdminToken, upload.single('logo'), async (req, res) => {
  try {
    const { name, career_page_url, description, industry, location } = req.body;
    
    // Validate required fields
    if (!name || !career_page_url) {
      return res.status(400).json({ error: 'Company name and career page URL are required' });
    }
    ...
    await req.db.query(`
      INSERT INTO companies (id, name, career_page_url, description, industry, location, logo_url, created_by)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `, [id, name, validatedUrl, description || null, industry || null, location || null, logoUrl, null]);
    
    logStep("ADMIN", `Company created successfully: ${id}`);
    ...
    logError("CAREER", "Error adding company through admin", error);
    res.status(500).json({ error: 'Failed to add company' });
  }
});

// Update an existing company
router.put('/admin/company/:id', verifyAdminToken, upload.single('logo'), async (req, res) => {
  try {
    const { id } = req.params;
    const { name, career_page_url, description, industry, location } = req.body;
    ...
    // Check if company exists
    const [companies] = await req.db.query('SELECT id, logo_url FROM companies WHERE id = ?', [id]);
    if (companies.length === 0) {
      return res.status(404).json({ error: 'Company not found' });
    }
    
    let logoUrl = companies[0].logo_url;
    ...
    await req.db.query(`
      UPDATE companies 
      SET name = ?, career_page_url = ?, description = ?, industry = ?, 
          location = ?, logo_url = ?, updated_at = NOW()
      WHERE id = ?
    `, [name, validatedUrl, description || null, industry || null, location || null, logoUrl, id]);
    
    logStep("ADMIN", `Company updated successfully: ${id}`);
    ...
    logError("CAREER", "Error updating company through admin", error);
    res.status(500).json({ error: 'Failed to update company' });
  }
});

// Delete a company
router.delete('/admin/company/:id', verifyAdminToken, async (req, res) => {
  try {
    const { id } = req.params;
    
    // Check if company exists
    const [companies] = await req.db.query('SELECT id FROM companies WHERE id = ?', [id]);
    if (companies.length === 0) {
      return res.status(404).json({ error: 'Company not found' });
    }
    
    // Delete company from database
    await req.db.query('DELETE FROM companies WHERE id = ?', [id]);
    
    logStep("ADMIN", `Company deleted successfully: ${id}`);
    ...
    logError("CAREER", "Error deleting company through admin", error);
